const MenuButton = {
  toggle() {
    document.querySelector(".add-category-button").classList.contains("active")
      ? MenuButton.close()
      : MenuButton.open()
  },
  open() {
...
const CategoryModal = {
  open() {
    document.querySelector("#category-modal").classList.add("active")

    DOM.categoryModal()
  },
  close() {
    document.querySelector("#category-modal").classList.remove("active")
  },
}
...
const UpdateTransactionModal = {
  open(index) {
    document.querySelector("#update-transaction-modal").classList.add("active")
    document
...
const OpeningBalanceModal = {
  open() {
    document.querySelector("#opening-balance-modal").classList.add("active")
  },
  close() {
    document.querySelector("#opening-balance-modal").classList.remove("active")
  },
}
...
const CardColor = {
  totalMonth: {
    positive() {
      document.querySelector(".card.total-month").classList.remove("negative")
      document.querySelector(".card.total-month").classList.add("positive")
    },
    negative() {
      document.querySelector(".card.total-month").classList.remove("positive")
      document.querySelector(".card.total-month").classList.add("negative")
    },
  },
  totalBalance: {
    positive() {
      document.querySelector(".card.total-balance").classList.remove("negative")
      document.querySelector(".card.total-balance").classList.add("positive")
    },
    negative() {
      document.querySelector(".card.total-balance").classList.remove("positive")
      document.querySelector(".card.total-balance").classList.add("negative")
    },
  },
}
...
const Calendar = {
  ...
  switchMonth(intention) {
    ...
    const currentMonthAlignment =
      switchMonth.offsetWidth / 2 - currentMonth.offsetWidth / 2
    currentMonth.style.transition =
      "left 300ms ease-in-out, right 300ms ease-in-out, opacity 150ms linear"
    currentMonth.style.opacity = "0"

    if (intention === "previous") {
      const previousMonthAlignment =
        switchMonth.offsetWidth / 2 - previousMonth.offsetWidth / 2
      previousMonth.style.transition =
        "left 300ms ease-in-out, opacity 150ms linear"
      ...
      const alignmentNextMonth =
        switchMonth.offsetWidth / 2 - nextMonth.offsetWidth / 2
      nextMonth.style.transition =
        "right 300ms ease-in-out, opacity 150ms linear"
...
const Storage = {
  ...
  getCategories() {
    return (
      JSON.parse(localStorage.getItem("dev.finances:categories")) || [
        "lazer",
        "carro",
        "casa",
        "trabalho",
...
const DOM = {
  ...
  innerHTMLTransaction(transactions, index) {
    ...
    const html = `
        <td onClick="${UpdateTransactionModal_func}" class="description">${
      transactions.description
    }</td>
        <td onClick="${UpdateTransactionModal_func}" class="description">${
      String(transactions.category)[0].toUpperCase() +
      String(transactions.category).substring(1)
    }</td>
        <td onClick="${UpdateTransactionModal_func}" class="${CSSclass}">${amount}</td>
        <td onClick="${UpdateTransactionModal_func}" class="date">${
      transactions.date
    }</td>
  ...
  updateTransactionModal(modalIndex) {
    const monthIndex = Calendar.activeMonth()
    const { category, description, amount, date, deposit } =
      Storage.getTransactions(monthIndex)[modalIndex]
    ...
    document.querySelector("#update-amount").value =
      Utils.formatSimpleAmountToText(String(amount))
  ...
  updateOpeningBalance() {
    document.querySelector("#openingBalanceDisplay").innerHTML =
      Utils.formatCurrency(Storage.getOpeningBalance())
    document.querySelector("#opening-balance-amount").value =
      Utils.formatSimpleAmountToText(Storage.getOpeningBalance())
  },
  updateBalance() {
  ...
  updateCalendar() {
    ...
    document.querySelector("#switch-previous-month-button").disabled =
      disabledPreviousMonth
    document.querySelector("#previous-month").innerHTML = disabledPreviousMonth
      ? ""
      : Calendar.months[previousMonth]
    document.querySelector("#current-month").innerHTML =
      Calendar.months[activeMonth]
    document.querySelector("#next-month").innerHTML = disabledNextMonth
      ? ""
      : Calendar.months[nextMonth]
    document.querySelector("#switch-next-month-button").disabled =
      disabledNextMonth
...
const UpdateTransactionForm = {
  ...
  validateFields() {
    const { category, description, amount, date } =
  ...
  formatValues() {
    let { category, description, amount, date, deposit } =
      UpdateTransactionForm.getValues()
...
const CreateCategoryForm = {
  ...
  validateFields() {
    ...
    if (categories.includes(category)) {
      throw new Error(
        "Esta categoria já existe, por favor preencha o campo com um valor diferente!"
      )
    }
  },
  formatValues() {
...
const DeleteCategoryForm = {
  ...
  validateFields() {
    ...
    if (!categories.includes(category)) {
      throw new Error(
        "Esta categoria não existe, por favor preencha o campo com um valor diferente!"
      )
    }
  },
  formatValues() {
...
const App = {
  ...
  initDOM() {
    Storage.getTransactions(Calendar.activeMonth()).forEach(
      (transactions, index) => DOM.addTransaction(transactions, index)
    )
